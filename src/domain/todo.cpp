...
TodoDefinitionResult TodoDefinition::create(const TodoInput& input) {
    ...
    if (static_cast<std::uint64_t>(input.priority) >
    ...
    return TodoDefinition(
        ...
        helpers::iso8601ToTimePoint(input.completedAt),
        input.completed);
}

TodoDefinition TodoDefinition::rehydrate(
    std::string title,
    ...
    bool completed) {
    return TodoDefinition(
        std::move(title), std::move(description), priority, completedAt, completed);
}
...
const std::string& TodoDefinition::title() const noexcept { return title_; }
const std::string& TodoDefinition::description() const noexcept { return description_; }
std::uint32_t TodoDefinition::priority() const noexcept { return priority_; }
const std::chrono::system_clock::time_point& TodoDefinition::completedAt() const noexcept { return completedAt_; }
bool TodoDefinition::completed() const noexcept { return completed_; }
