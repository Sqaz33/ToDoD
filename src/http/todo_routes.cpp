...
std::optional<std::int32_t> parseInt32(std::string_view text) {
    ...
    if (result.ec != std::errc{} || result.ptr != text.data() + text.size() ||
        value < 0 || value > std::numeric_limits<std::int32_t>::max()) return std::nullopt;
...
std::optional<domain::TodoInput> parseTodoInput(const crow::json::rvalue& json) {
    if (!json.has("title") || !json.has("description") || !json.has("priority") ||
        !json.has("completedAt") || !json.has("completed")) return std::nullopt;
    if (json["title"].t() != crow::json::type::String ||
        json["description"].t() != crow::json::type::String ||
        json["completedAt"].t() != crow::json::type::String) return std::nullopt;
    const auto completedType = json["completed"].t();
    if (completedType != crow::json::type::True && completedType != crow::json::type::False) return std::nullopt;
...
void registerTodoRoutes(crow::SimpleApp& app, use_cases::TodoUseCases& useCases) {
    CROW_ROUTE(app, "/api/page").methods(crow::HTTPMethod::GET)
    ([&useCases](const crow::request& request) {
        ...
        if (!offsetValue || !limitValue) return errorResponse(400, "INVALID_PAGINATION", "offset and limit are required");
        auto offset = parseInt32(offsetValue);
        auto limit = parseInt32(limitValue);
        if (!offset || !limit) return errorResponse(400, "INVALID_PAGINATION", "offset and limit must be non-negative int32 values");

        auto page = useCases.getPage(*offset, *limit);
        if (!page) return errorResponse(500, "INTERNAL_ERROR", "Internal server error");

        crow::json::wvalue json;
        crow::json::wvalue::list todos;
        for (const auto& todo : page->items) todos.push_back(todoToJson(todo));
    ...
    ([&useCases](const crow::request& request) {
        ...
        const bool allSucceeded = std::ranges::all_of(result->handlers, [](const auto& handler) {
    ...
    CROW_ROUTE(app, "/api/todos/<string>").methods(crow::HTTPMethod::GET)
    ([](const std::string&) { return notImplementedResponse(); });
    CROW_ROUTE(app, "/api/todos/<string>").methods(crow::HTTPMethod::PATCH)
