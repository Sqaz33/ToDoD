...
HandlerScriptService::HandlerScriptService(repository::TodoRepository& todoRepository,
                                           repository::ScriptRepository& scriptRepository,
                                           db::DataBase& database,
                                           scripting::engine::ScriptEngine& scriptEngine)
    : todoRepository_(todoRepository), scriptRepository_(scriptRepository), database_(database),
      scriptEngine_(scriptEngine) {}

RunHandlersResult HandlerScriptService::runHandlers(const domain::TodoTask& todo,
    ...
    for (const auto& handler : *handlers) {
        ...
        if (engineResult.error) {
            ...
            result.status = code == scripting::error::ScriptErrorCode::InstructionLimitExceeded ||
            ...
            try {
                database_.transaction([&](db::DBAccess& access, bool* commit) {
                    ...
                    for (std::size_t index = 0; index < engineResult.context.commands.size();
                         ++index) {
                        auto commandResult =
                            runCommand_(engineResult.context.commands[index], access, index);
                ...
                result.status = HandlerExecutionStatus::CommandError;
                result.storageError =
                    db::error::StorageError::create("execute handler transaction", exception);
                for (auto& commandResult : result.commandResults) {
                    if (commandResult.status == CommandStatus::Applied) {
                        commandResult.status = CommandStatus::RolledBack;
                    }
...
CommandResult HandlerScriptService::runCommand_(const scripting::api::ScriptCommand& command,
                                                db::DBAccess& access, std::size_t index) {
