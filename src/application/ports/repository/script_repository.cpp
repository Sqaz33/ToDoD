...
ScriptRepository::ScriptRepository(std::shared_ptr<db::DataBase> db)
    : db_(std::move(db)),
      ...
      findByEventQuery_(db_->connection(), "SELECT id, name, source, event, enabled FROM scripts "
                                           "WHERE event = ? AND enabled = 1 ORDER BY id") {}
...
GetHandlersResult ScriptRepository::getAll(db::DBAccess&) {
    ...
    try {
        ...
        while (getAllQuery_.executeStep())
            handlers.push_back(readScript(getAllQuery_));
        return handlers;
    } catch (const SQLite::Exception& exception) {
...
FindHandlerScriptByEventResult ScriptRepository::findByEvent(domain::TodoEvent event,
    ...
    try {
        ...
        while (findByEventQuery_.executeStep())
            handlers.push_back(readScript(findByEventQuery_));
        return handlers;
    } catch (const SQLite::Exception& exception) {
        return std::unexpected(
            db::error::StorageError::create("find handlers by event", exception));
    }
}
