...
ExecutionResult ScriptEngine::execute(const domain::HandlerScript& script,
    ...
    auto&& log = [&context](const std::string& message) { context.logs.push_back(message); };

    auto error = execution::execute(
        script.def.source(), "todo",
        std::tuple{execution::named("id", todo.id.id), execution::named("title", todo.def.title()),
        ...
        std::tuple{execution::named("complete", complete),
                   execution::named("set_priority", setPriority), execution::named("log", log)},
